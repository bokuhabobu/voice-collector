...
3. 画面の文章を読み上げながら「録音開始」を押し、読み終わったら「停止」➔「✓ 保存して次の文へ」を押します。
4. 全ての録音が完了すると自動で提出案内が開きます（途中で終える場合は右上の「📤 録音を提出する」を押します）。
5. 「✉️ メールで提出する」を押すと共有メニューが開くので、メールアプリ（Gmailや標準メール等）を選んでそのまま送信してください。

※ アカウント登録やアプリのインストールは一切不要です。
```
